...
class PaymentIntent
{
    ...
    public function loadFromCache($quote, $order)
    {
        if (empty($quote))
            return null;
        ...
        if ($this->isInvalid($quote, $order) || $this->hasPaymentActionChanged())
        {
    ...
    public function create($quote, $payment = null, $order = null, $paymentMethodId = null)
    {
        if (!$this->config->isEnabled())
            return null;

        $this->getParamsFrom($quote, $payment, $order, $paymentMethodId);
        ...
        if ($this->helper->isMultiShipping())
            ...
            $this->loadFromCache($quote, $order);

        $this->getParamsFrom($quote, $payment, $order, $paymentMethodId); // We re-run it in case we destroyed the old PI
    ...
    public function addPaymentMethodDetails($quote, $paymentMethodId, $payment = null, $order = null)
    {
        $save = ($this->helper->isMultiShipping() || ($payment && $payment->getAdditionalInformation("save_card")));
        $this->params['payment_method'] = $paymentMethodId;

        if ($save)
        {
            $this->customer->createStripeCustomerIfNotExists(true, $order);
            $this->params['save_payment_method'] = true;
        }

        if ($this->customer->getStripeId())
            $this->params["customer"] = $this->customer->getStripeId();

        if ($this->config->isInstallmentPlansEnabled())
            $this->params["payment_method_options"]["card"]["installments"]["enabled"] = true;

        return $this->params;
    }

    public function getParamsFrom($quote, $payment = null, $order = null, $paymentMethodId = null)
    {
        if ($order)
        {
        ...
        if ($paymentMethodId)
            $this->addPaymentMethodDetails($quote, $paymentMethodId, $payment, $order);

        if ($order)
        {
            $this->params["description"] = $this->helper->getOrderDescription($order);
            $this->params["metadata"] = $this->config->getMetadata($order);
        }

        $this->adjustAmountForSubscriptions($quote, $order);
        ...
        if ($customerEmail)
            $this->params["receipt_email"] = $customerEmail;

        return $this->params;
    }

    // Adds initial fees, or removes item amounts if there is a trial set
    protected function adjustAmountForSubscriptions($quote, $order = null)
    {
    ...
    public function isInvalid($quote, $order)
    {
    ...
    public function updateFrom($quote, $order)
    {
        if (empty($quote))
            return $this;
        ...
        if ($this->isSuccessful(false, $order))
            return $this;
    ...
    public function isSuccessful($fetchFromAPI = true, $order)
    {
        if (!$this->config->isEnabled())
            return false;
    ...
    public function confirmAndAssociateWithOrder($order, $payment)
    {
        ...
        $created = $this->create($quote, $payment, $order, $payment->getAdditionalInformation("token")); // Load or create the Payment Intent
        ...
        if (!$this->isSuccessfulStatus())
        {
            ...
            // This should only trigger with Apple Pay
            if (empty($this->paymentIntent->description))
            {
                $save = ($this->helper->isMultiShipping() || $payment->getAdditionalInformation("save_card"));
                $this->setPaymentMethod($payment->getAdditionalInformation("token"), $save, false, $order);
                $this->paymentIntent->description = $this->helper->getOrderDescription($order);
                $this->paymentIntent->metadata = $this->config->getMetadata($order);
                $this->updatePaymentIntent($quote);
            }
            ...
            if ($installmentPlan = $this->getInstallmentPlan($payment))
    ...
    public function setPaymentMethod($paymentMethodId, $save = false, $update = true, $order = null)
    {
        ...
        if (!$this->helper->isMultiShipping() && !$this->loadFromCache($quote, $order))
            return $this;
